...
public class Sprite extends Layer {
	 ...
	 * <p>
	 * Changing the reference pixel does not change the Sprite's physical
	...
	/**
	 * Sets the transform for this Sprite. Transforms can be applied to a Sprite
	 * to change its rendered appearance. Transforms are applied to the original
	 * Sprite image; they are not cumulative, nor can they be combined. By
	 * default, a Sprite's transform is {@link #TRANS_NONE}.
	 * <P>
	 * Since some transforms involve rotations of 90 or 270 degrees, their use
	 * may result in the overall width and height of the Sprite being swapped.
	 ...
	 * remains static relative to the pixel data of the Sprite. Similarly, the
	 * defined reference pixel is unchanged by this method, but its visual
	 * location within the Sprite may change as a result.
	 * <P>
	 * This method repositions the Sprite so that the location of the reference
	 ...
	 * pixels collide. That is, an opaque pixel in the first Sprite would have
	 * to collide with an opaque pixel in the second Sprite for a collision to
	 ...
	 * 
	 * @param s
	 *            the <code>Sprite</code> to test for collision with
	 * @param pixelLevel
	 ...
	 *         <code>false</code>
	 * @throws NullPointerException
	 *             if Sprite <code>s</code> is <code>null</code>
	 */
	 ...
	 * If pixel-level detection is used, a collision is detected only if opaque
	 * pixels collide. That is, an opaque pixel in the Sprite would have to
	 * collide with an opaque pixel in TiledLayer for a collision to be
	 * detected. Only those pixels within the Sprite's collision rectangle are
	 * checked.
	 * <P>
	 * If pixel-level detection is not used, this method simply checks if the
	 ...
	 * <P>
	 * The Sprite and the TiledLayer must both be visible in order for a
	 * collision to be detected.
	 * <P>
	 * 
	 * @param t
	 *            the <code>TiledLayer</code> to test for collision with
	 * @param pixelLevel
	 *            <code>true</code> to test for collision on a pixel-by-pixel
	 *            basis, <code>false</code> to test using simple bounds checking
	 *            against non-empty cells.
	 * @return <code>true</code> if this <code>Sprite</code> has collided with
	 *         the <code>TiledLayer</code>, otherwise <code>false</code>
	 * @throws NullPointerException
